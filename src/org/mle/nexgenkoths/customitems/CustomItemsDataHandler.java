        ...
        for(String customItemName : keys) {
            ...
            for(String enchantStr : sect.getStringList("Enchantments")) {
                ...
                if(enchSplit.length != 2) {
                    P.log(Level.WARNING, "Length of string \"" + enchantStr + "\" when split by \"\\:\" isn't 2.  Ignoring enchantments entry for Custom Item \"" + customItemName + "\".");
                    continue;
                }
                ...
                if(ench == null) {
                    P.log(Level.WARNING, "Enchantment \"" + enchSplit[0] + "\" doesn't exist. Ignoring enchantments entry for Custom Item \"" + customItemName + "\".");
                    continue;
                }
                
                if(!NumberUtils.isInteger(levelStr)) {
                    P.log(Level.WARNING, "String \"" + levelStr + " is not a valid Integer. Ignoring enchantments entry for Custom Item \"" + customItemName + "\".");
                    continue;
                }
            ...
            if(material == null) {
                P.log(Level.WARNING, "Item Name \"" + sect.getString("Item") + " is not valid (Loading Custom Item: \"" + customItemName + "\"). Ignoring item.");
                continue;
            }
